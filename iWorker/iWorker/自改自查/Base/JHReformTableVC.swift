...
// MARK: - JHReformTableNavVC

@objcMembers open class JHReformTableNavVC: TableUserInterface {
    ...
    // MARK: - Life
    /**
            UITableViewController 根视图View是UITableView，无法正常显示金和的导航条。
     */
...
//        self.tableView.addSubview(navBar)
    ...
    lazy public var navBar: JHReformTableNavBar = {
        let tmpView = JHReformTableNavBar.init(frame: .zero)
        tmpView.backBtn.addTarget(self, action: #selector(backBtnClicked(_:)), for: .touchUpInside)
        return tmpView
    }()
    
    lazy public var emptyView: JHReformTableEmptyView = {
        let tmpView = JHReformTableEmptyView.init(frame: .zero)
...
// MARK: - JHReformTableNavBar

@objcMembers public class JHReformTableNavBar: UIView {
...
// MARK: - JHReformTableEmptyView

@objcMembers public class JHReformTableEmptyView: UIView {
