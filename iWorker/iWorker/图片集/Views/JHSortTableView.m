...
//  JHLivePlayLibrary
//
//  Created by 杨智远 on 2019/9/25.
//  Copyright © 2019 pk. All rights reserved.
//

#import "JHSortTableView.h"
...
@implementation JHSortTableView
...
- (void)exchangeCell:(CGPoint)point {
    ...
    if (self.isGroup) {
        ...
        NSMutableArray *toSection = self.dataArray[toIndexPath.section];
        id obj = fromSection[_fromIndexPath.row];
        fromSection[_fromIndexPath.row] = toSection[toIndexPath.row];
        toSection[toIndexPath.row] = obj;
    } else {
        [self.dataArray exchangeObjectAtIndex:_fromIndexPath.row withObjectAtIndex:toIndexPath.row];
    }
    [self reloadData];
