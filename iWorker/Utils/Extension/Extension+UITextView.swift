...
extension UITextView {
    ...
    fileprivate struct RuntimeKey {
        ...
        static let oldFrame: UnsafeRawPointer! = UnsafeRawPointer.init(bitPattern: "OLDFRAME".hashValue)
    ...
    fileprivate func initPlaceholder(_ placeholder: String) {
        ...
        let rect = placeholder.boundingRect(with: CGSize(width: frame.size.width - placeholderOrigin!.x * 2, height: CGFloat(MAXFLOAT)), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : self.placeholdFont!], context: nil)
        placeholderLabel?.frame = CGRect(x: placeholderOrigin!.x, y: placeholderOrigin!.y, width: frame.size.width - placeholderOrigin!.x * 2, height: rect.size.height)
...
//                JKPrint("autoHeight的CGPoint=========\(weakSelf.placeholderOrigin!)")
                let xx = weakSelf.frame.origin.x
                let yy = weakSelf.frame.origin.y
                let ww = weakSelf.frame.size.width
                let more = size.height + weakSelf.placeholderOrigin!.y * 2 > weakSelf.oldFrame!.height
                var hh = weakSelf.oldFrame!.height
                if more {
                    hh = size.height // + weakSelf.placeholderOrigin!.y * 2
                }
//                weakSelf.frame = CGRect(x: xx, y: yy, width: ww, height: hh)
